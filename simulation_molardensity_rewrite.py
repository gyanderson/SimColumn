...
def congener_mole_fraction_float(congener,ethanol,water):
    ...
    if (congener + ethanol*ethanol_density/ethanol_MW + water*water_density/water_MW) < eps or ethanol < eps or water < eps:
        congener_mole_fraction = eps
    elif congener < eps:
        congener_mole_fraction = eps
    elif (congener / (congener + ethanol*ethanol_density/ethanol_MW + water*water_density/water_MW)) < eps:
        congener_mole_fraction = eps
    else:
...
def congener_molarity_float(congener,ethanol,water):
    if (ethanol + water) < eps or ethanol < eps or water < eps:
        congener_molarity = eps # Could change this to the actual molarity of the pure congener?
    elif congener < eps:
        congener_molarity = eps
    else:
        congener_molarity = congener/(ethanol + water)
    return congener_molarity #mol/mL
...
def initialize(ethanol,water,plates,congener):
    ...
    for i in range(0,(plates*3),3):
        ...
        col0.append(5.0)
        col0.append(5.0)
        col0.append(congener/100.0)
    col0[0:3] = [ethanol, water, congener*(100.0-(plates-1.0))/100.0] # initialize the pot
...
def col(t,y,plates,congener_identity,reflux_knob,power):
    ...
    for i in range(0,(len(dydt)-3),3):
        ...
        if i == 0: # Pot
            ...
            if y[i+2] > eps:
                dydt[i+2] += - congener_molar_density*(congener_partial_pressure/1.01325)*vap_rate# current plate congener
            if y[i] > eps:
                dydt[i+3] += vapor_abv*vap_rate # upper plate ethanol
            if y[i+1] > eps:
                dydt[i+4] += (1.0-vapor_abv)*vap_rate # upper plate water
            if y[i+2] > eps:
                dydt[i+5] +=   congener_molar_density*(congener_partial_pressure/1.01325)*vap_rate # upper plate congener
            ...
            if y[i+2] > eps:
                dydt[i+2] += -congener_molarity*reflux_rate - congener_molar_density*(congener_partial_pressure/1.01325)*vap_rate # current plate congener
...
def distill(ethanol,water,plates,congener,congener_identity,reflux_knob,power,timestep,tend):
    ...
    t0 = 0
    solver = ode(col).set_integrator('vode',method='bdf',order=1,atol=0.05,rtol=0.05)
    ...
    for i in range(plates):
        if i == 0:
            ...
            name = 'plate ' + str(i)
        plt.subplot(plates,3,(i*3)+1)
        plt.plot(t,sol[:,(i*3)]/(sol[:,(i*3)] + sol[:,(i*3)+1]), 'b', label='abv|' + name)
        plt.legend(loc='best')
        plt.xlabel('t')
        plt.grid()

        plt.subplot(plates,3,(i*3)+2)
        plt.plot(t,(sol[:,(i*3)] + sol[:,(i*3)+1]), 'g', label='Volume (mL)|' + name)
        plt.legend(loc='best')
        plt.xlabel('t')
        plt.grid()

        plt.subplot(plates,3,(i*3)+3)
        plt.plot(t,sol[:,(i*3)+2]*1000000,'r', label='Congener (umol)|' + name)
        plt.legend(loc='best')
        plt.xlabel('t')
        plt.grid()
